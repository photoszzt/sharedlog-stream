...
// Complexity: O(1).
func (t *TreeMap) Len() uint64 { return t.count }
...
func (t *TreeMap) ReverseRange(from, to Key) (ReverseIterator, ReverseIterator) {
...
func (t *TreeMap) ReverseUpperBound(key Key) ReverseIterator {
	result := t.endNode
	node := t.endNode.left
	if node == nil {
		return ReverseIterator{tree: t, node: t.endNode}
	}
	for {
		if t.Less(node.key, key) { // node.key < key
			...
			if node.left != nil {
				...
				return ReverseIterator{tree: t, node: result}
			}
		}
	}
}
...
func (t *TreeMap) ReverseLowerBound(key Key) ReverseIterator {
	result := t.endNode
	node := t.endNode.left
	if node == nil {
		return ReverseIterator{tree: t, node: t.endNode}
